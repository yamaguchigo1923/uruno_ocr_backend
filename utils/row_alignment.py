...
def align_rows_by_number(
    ...
    出力:
        ...
        - pairs: (gi, oj) のペアリスト。gi/oj は 0 始まりインデックス。
        - skipped_gt_indices: 正解側でスキップされた gi のインデックス（昇順）
        - skipped_ocr_indices: OCR側でスキップされた oj のインデックス（昇順）
    """
    ...
    for i in range(1, n + 1):
        for j in range(1, m + 1):
            match_score = dp[i - 1][j - 1] + score_number(
                gt_numbers[i - 1], ocr_numbers[j - 1]
            )
